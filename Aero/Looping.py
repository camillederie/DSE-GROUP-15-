...
cambers = np.arange(0.04, 0.17, 0.02)
CD_loop = []
CL_loop = []
LD_loop = []
Power_loop = []
...
if Polars == True:
    for i in range(len(cambers)):
        k = cambers[i]
        fig = plt.figure(figsize=(6, 5))

        aoa_0 = np.arange(0, 19.5, 1.5)
        CD_x = []
        CL_x = []
    ...
    for i in range(len(CL_loop)):
        ax1.plot(aoa_0, LD_loop[i], linestyle=linestyles[i], label=f'camber = {round(cambers[i], 2)}')
        ax2.plot(aoa_0, Power_loop[i], linestyle=linestyles[i], label =f'camber = {round(cambers[i], 2)}')
        ax3.plot(aoa_0, CL_loop[i], linestyle=linestyles[i], label =f'camber = {round(cambers[i], 2)}')
        ax4.plot(aoa_0, CD_loop[i], linestyle=linestyles[i], label =f'camber = {round(cambers[i], 2)}')
